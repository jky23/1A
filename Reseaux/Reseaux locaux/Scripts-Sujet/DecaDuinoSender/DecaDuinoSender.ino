...
void setup()
{
  ...
  if (!decaduino.setTxPcode(4)){
    Serial.println("setTxPcode failed");
  }
   if (!decaduino.setRxPcode(4)){
    Serial.println("setRxPcode failed");
  }
...
void loop()
{
  ...
  // Envoi de la trame
  decaduino.pdDataRequest(txData, txLen+3);
  // On attend que la trame soit émise
  while ( !decaduino.hasTxSucceeded() );
  // Log du nb de trames émises
  digitalWrite(13, LOW);

  //Attendre de recevoir un ACK
  decaduino.plmeRxEnabledRequest();
  ...
  while (i < 200 && !ack_ok) {
    ...
    if (!decaduino.rxFrameAvailable()) {
      if (rxData[0] == 0) {
        if (rxData[1] == txFrames) {
          ++txFrames;
          ack_ok = true;
        } else {
          // pas le bon ack (en theorie pas possible)
        }
      } else {
