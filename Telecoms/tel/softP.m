function [tebs_softP,bitsDecodes_soft] = softP(bits,N, Nb_erreur_voulu, rsbs_dB, nb_bit_max)%Constantes
...
for rsb_dB=rsbs_dB % On parcours le RSB
    ...
    while ((Nb_erreur < Nb_erreur_voulu)&&(Nb_passage < (nb_bit_max)/(2*N)))
       % bits = randi([0 1],1,2*N); %suite aleatoire de bits (QPSK)
        ...
        % codage convolutif
        bitsEncodes = convenc(bits, trellis, p_poinc);
    ...
    tebs_softP = [tebs_softP, Nb_erreur/(Nb_passage*2*N)]
end
end
