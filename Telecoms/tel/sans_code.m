function [tebs_sans_code,signal_dk] = sans_code(bits,N, Nb_erreur_voulu, rsbs_dB, nb_bit_max)%Constantes
...
%         bitsEncodes = convenc(bits, trellis);
        ...
        % Mapping
        symboles = 1 - bits*2;        % Suite de symbole
...
%         bitsDecodes_soft = vitdec(r_e_decoupe, trellis, 5*k, 'trunc', 'unquant');
        ...
        Nb_erreur = Nb_erreur + sum(signal_dk ~= bits);
        Nb_passage = Nb_passage + 1;
    end
    tebs_sans_code = [tebs_sans_code, Nb_erreur/(Nb_passage*2*N)]
end
end
