...
% 1ere chaine
  ...
  plot(z_til_1)
  xlabel('Periode Ts')
  title("Diagramme de l'oeil de la chaine 3")
  
  % Echantillonnage
  % Pour l'echantillonage on choisit t0 = 3*Ts/4
  % Car l'oeil est ouvert pour t = 3*Ts/4
  t0 = 3*Ts/4;
  dec_1 = z_1(t0:Ts:end);
  ...
  for i = 0 : 6
      ...
      TEB_Theo(i+1) = 2*(M-1)/M.*qfunc(sqrt(Eb_No)); % Mapping polaire, mais filtrage non adapté
     % Rajout du bruit
