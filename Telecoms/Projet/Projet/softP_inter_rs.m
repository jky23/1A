...
bitsEncodes = convenc(bits_entrelaces, trellis, P);
...
for rsb_dB=rsbs_dB % On parcours le RSB
    %     Nb_erreur_voulu = 1*length(bits)/1000;
    %     Nb_erreur = 0;
    %     Nb_passage = 0;
    %
    ...
    for i=1:5
        ...
        teb_tmp(i) = mean(bits_decodes_RS ~= bits);
        %taux d'erreur binaire
        %Eb_No = 10^(i/10);
        teb_tmp_theo(i) =  2*(1-normcdf(sqrt(2*exp((rsb_dB/10)*log(10)))));
    end
    ...
    %     % end
    tebs_softP_inter_rs = [tebs_softP_inter_rs, tmp_val];
