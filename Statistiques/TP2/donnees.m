...
pas = 0.01;
x_droite_reelle = transpose(-taille:pas:taille);
y_droite_reelle = transpose(-taille:pas:taille);
...
n = 50;
if droite_horizontale
	x_donnees = taille*(2*rand(n,1)-1);
	y_donnees = (rho_0-cos_theta_0*x_donnees)/sin_theta_0;
else
	y_donnees = taille*(2*rand(n,1)-1);
	x_donnees = (rho_0-sin_theta_0*y_donnees)/cos_theta_0;
end

% Donnees bruitees :
sigma = 2;
x_donnees_bruitees = x_donnees+sigma*randn(n,1);
y_donnees_bruitees = y_donnees+sigma*randn(n,1);
