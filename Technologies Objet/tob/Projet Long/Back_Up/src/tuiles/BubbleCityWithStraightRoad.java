...
public class BubbleCityWithStraightRoad extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Bubble_City_With_Straight_Road.png";

	public BubbleCityWithStraightRoad(int row, int col) {
		super(IMAGE_PATH, row, col);
	}
	
	private BubbleCityWithStraightRoad() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		objects.add(new Champs(temp, new Point(LARGEUR_TUILE / 2, HAUTEUR_TUILE / 3), Arrays.asList(city1)));
		...
		objects.add(new Champs(temp, new Point(LARGEUR_TUILE / 2, HAUTEUR_TUILE * 7 /8), new ArrayList<Ville>()));
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new BubbleCityWithStraightRoad(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new BubbleCityWithStraightRoad());
	}
}
