...
public class BubbleCityWithCurvedRoadTwo extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Bubble_City_With_Curved_Road_Two.png";

	public BubbleCityWithCurvedRoadTwo(int row, int col) {
		super(IMAGE_PATH, row, col);
	}
	
	private BubbleCityWithCurvedRoadTwo() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		objects.add(new Champs(temp, new Point(LARGEUR_TUILE / 3, HAUTEUR_TUILE * 2 / 3), Arrays.asList(city1)));
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new BubbleCityWithCurvedRoadTwo(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new BubbleCityWithCurvedRoadTwo());
	}
}