...
public class GatehouseWithShield extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Gatehouse_With_Shield.png";
	
	public GatehouseWithShield(int row, int col) {
		super(IMAGE_PATH, row, col);
	}

	private GatehouseWithShield() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		Ville city1 = new Ville(temp, new Point(LARGEUR_TUILE /2, HAUTEUR_TUILE / 4), true);
		objects.add(city1);
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new GatehouseWithShield(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new GatehouseWithShield());
	}
}
