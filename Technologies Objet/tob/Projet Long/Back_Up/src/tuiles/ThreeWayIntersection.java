...
public class ThreeWayIntersection extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Three_Way_Intersection.png";
	
	public ThreeWayIntersection(int row, int col) {
		super(IMAGE_PATH, row, col);
	}
	
	private ThreeWayIntersection() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		objects.add(new Champs(temp, new Point(LARGEUR_TUILE / 2, HAUTEUR_TUILE / 4), new ArrayList<Ville>()));
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new ThreeWayIntersection(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new ThreeWayIntersection());
	}
