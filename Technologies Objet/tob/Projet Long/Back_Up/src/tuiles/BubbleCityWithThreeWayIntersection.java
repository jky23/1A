...
public class BubbleCityWithThreeWayIntersection extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Bubble_City_With_Three_Way_Intersection.png";
	
	public BubbleCityWithThreeWayIntersection(int row, int col) {
		super(IMAGE_PATH, row, col);
	}
	
	private BubbleCityWithThreeWayIntersection() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		objects.add(new Champs(temp, new Point(LARGEUR_TUILE / 2, HAUTEUR_TUILE / 4), Arrays.asList(city1)));
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new BubbleCityWithThreeWayIntersection(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new BubbleCityWithThreeWayIntersection());
	}
