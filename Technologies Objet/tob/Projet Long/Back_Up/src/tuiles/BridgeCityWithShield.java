...
public class BridgeCityWithShield extends VueTuileAbstraite {
	private static final String IMAGE_PATH = "src/resources/Bridge_City_With_Shield.png";

	public BridgeCityWithShield(int row, int col) {
		super(IMAGE_PATH, row, col);
	}
	
	private BridgeCityWithShield() {}
	...
	public List<TuileAbstraite> construireObjetListe() {
		...
		Ville city1 = new Ville(temp, new Point(LARGEUR_TUILE / 2, HAUTEUR_TUILE / 2), true);
		objects.add(city1);
	...
	public VueTuileAbstraite construireTuile(int row, int col) {
		return new BridgeCityWithShield(row, col);
	}
	
	public static CreateurTuile getConstruction() {
		return new CreateurTuile(new BridgeCityWithShield());
	}
}