...
void effacer(int i) { 
	/* efface le descripteur pour le participant i */
...
int main (int argc, char *argv[]) {
    ...
    while (true) {
		...
		/* boucle du serveur : traiter les requêtes en attente 
					* sur le tube d'écoute : ajouter les nouveaux participantset les tubes d'entrée
					  Note :  il est possible que nbactifs dépasse MAXPARTICIPANTS : dans le cas 
            où le tube d'écoute est plein, il peut contenir TAILLE_RECEPTION/sizeof(demande)
             requêtes.Le nombre maximum effectif de participants est donc
            MAXPARTICIPANTS + TAILLE_RECEPTION/sizeof(demande).
           ...
           à gér1er. Enfin, on ne traite pas plus de TAILLE_RECEPTION/TAILLE_MSG*sizeof(char)
           à chaque fois.
