...
cos_alpha = cos(alpha);
S = 2/sin(alpha);			% Seuil sur la norme du gradient (decoule d'une etude theorique)

% Lecture de l'image :
% I = imread('Images/Morlaix.png');
%I = imread('Images/Piree.png');
% I = imread('Images/chaises.png');
I = imread('Images/Joel.png');
if size(I,3)==3
	I = rgb2gray(I);
end
I = imresize(I,0.8);
...
G_x_n = G_x./G_norme;							% Gradient normalise
G_y_n = G_y./G_norme;
while length(indices_contour)>0
	...
	[E,contour] = recursion(E,contour,i_germe,j_germe,voisins,G_x_n,G_y_n,card_max,cos_alpha);
