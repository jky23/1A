...
!   with Rayleigh-Ritz projection + block approach + deflation (TO BE DEVELOPPED)
!
!--------------------------------------------------------------------------------------------------------------------
!
!! subspace_iter_v2
...
! using the subspace iteration method with Rayleigh-Ritz projection + block approach + deflation (v2)
!
! ..Input arguments
...
!-------------------------------------------------------------------------------------------------------------------
  subroutine subspace_iter_v2(n, m, a, p, percentage, maxit, eps, v, w, n_ev, acc_ev, it_ev, ierr)
    ...
    double precision, dimension(n, n)                :: temp
    double precision, dimension(n, n)                :: temp2
    ...
    temp = a
    do j = 1, p
        call dgemm('n', 'n', n, n, n, done, temp, n, a, n, dzero, temp2, n)
        temp = temp2
    end do
    ...
    do while((eig_sum .lt. p_trace) .and. (n_ev .lt. m) .and. (k .lt. maxit))
      ...
      !! compute  y = a^p*v
      call dgemm('n', 'n', n, m, n, done, temp, n, v, n, dzero, y, n)
