...
subroutine facto(m, ldm, n, ipivot, p, q, ierr)
  ...
  !  -----------------
  !      Indices de boucle
  integer :: i,j,k, expo_det

  double precision :: eps, somme, frac_det
  ...
  if (ipivot.eq.0) then
	 write(*,*) 'Factorisation sans pivot'
	 do k = 1, n-1
	    if (abs(m(k,k)).le.(eps*norme_f(m(ldm,n),ldm,n))) then
	       ierr = 1
	       write(*,*) ierr
	    end if
	    ...
	    somme = 0
	    do j = k+1, n
	       somme = somme + m(j,j-1)*m(j-1,j)
	    end do
	    m(k+1:n,k+1:n) = m(k+1:n,k+1:n) - somme  
	 end do
   
   !if (ipivot.e.1) then
    !  write(*,*) 'Factorisation avec pivot partiel'
      
     ! do k = 1, n-1
      !  if (abs(m(k,k)).le.(eps*norme_f(m(ldm,n)))) then
	   !    ierr = 1
	    !   return ierr
	    !end if
	    !p(k) = 
  ...
  expo_det = 0
  do k = 1, n
     frac_det = fraction(m(k,k)) * frac_det
     frac_det = fraction(frac_det)
     expo_det = exponent(m(k,k)) + expo_det
  end do
  write(6,*) " =============================================="
  write(6,*) " Fin de la FACTORISATION " 
  write(6,*) " ========================" 
  write(6,*) " ... AFFICHER DETERMINANT de la matrice "
  write(6,*) frac_det ,"* 2 exposant", expo_det 
...
subroutine analyse_erreur(m,ldm,n,b,x,r,xexact)
  ...
  !      Indices de boucles
  integer :: i, j
  double precision :: erreur, residu
...
!! Calcul et afffichage de la norme du résidu 
  ...
  z = b - z
  residu = norme(z,n)
  residu = residu/abs((norme_f(m,ldm,n) * norme(x,n)) + norme(b,n))
  
  erreur = norme((x - xexact),n)/norme(xexact,n)
  write(6,*) "L'erreur relative est", erreur
  write(6,*) "La norme du residu est", residu
